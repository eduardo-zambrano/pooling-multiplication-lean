...
`a ⋆ j`, for positive integers `j`, are the **⋆-multiples of `a`**. For a
nonnegative integer **threshold** `N`, define

$$
M_a(N)=\#\{j\geq1:a\star j\leq N\}.
$$

Under strict row order, these entries are distinct, so `M_a(N)` counts the
⋆-multiples of `a` not exceeding `N`. Saying that a threshold contains at
least `u` ⋆-multiples means `M_a(N) ≥ u`.

Two principles describe what happens when thresholds are added. The base
`a` stays the same, `N,L` are nonnegative integer thresholds, and `u,v` are
positive integer target counts:

- **Sufficiency is preserved (PS).** If `N` contains at least `u`
  ⋆-multiples of `a`, and `L` contains at least `v`, then `N + L` contains
  at least `u + v`. Equivalently,
  `M_a(N + L) ≥ M_a(N) + M_a(L)`.
- **Insufficiency is preserved (PI).** If `N` contains fewer than `u`
...
frequency such as `M_a(N) = floor(N/a)`. They concern counts of ⋆-multiples,
not factorizations into atoms. Pooling adds the numerical thresholds;
it does not take the union of the two sets of ⋆-multiples, which are
nested.
...
Strict order lets counts locate row entries: `M_a(N) ≥ b` exactly when
the `b`-th ⋆-multiple, `a ⋆ b`, does not exceed `N`. The two pooling
principles therefore become
...
| Paper result | Module |
|---|---|
| Counts of ⋆-multiples via positive row indices, finiteness, threshold inversion | `Counting` |
| Lemma 3.1: counting inequalities, carry, counts of ⋆-multiples in intervals | `Counting`, `Supplement` |
...
The current manuscript also contains a uniform bounded-defect observation
(Remark 4.4). That extension is **not included in the formalization**.
Interpretive discussions, claims of novelty, and results from cited papers
are outside its scope. The concluding question about aggregate two-factor
counts is a research direction, not a verified result.
...
- **Actual cardinalities.** `count` uses `Set.ncard` of the full set of
  positive row indices `j` such that `op a j ≤ N`. There is no unproved
  count oracle or assumed finite cutoff. Finiteness is proved from
  positivity and strict order. Strict order also makes the row injective,
  so this equals the number of distinct ⋆-multiples not exceeding `N`;
  `outputCount_eq_count` in
  [`Supplement.lean`](PoolingMultiplication/Supplement.lean) proves that
  equality explicitly. For the nonmonotone XOR example, a separate
  finite-image argument proves its count; strict-order lemmas are not
  applied to it.
- **Target counts.** Lean's `PS` and `PI` use positive natural-number
  targets named `b,c`, corresponding to `u,v` in the prose. `PS` is
  implemented in its sufficiency-implication form;
  `ps_iff_count_superadd` proves its equivalence to the displayed count
  inequality, including thresholds whose counts are zero.
